...
With OAuth 1.0, the process of registering a third-party application for access usually begins with the third-party app developer submitting details of their app's purpose and requirements for review by the first-party site.

Once registered, the first-party site would provide the app with a Consumer API Key and a Consumer Secret Key. When a user wishes to authorize the third-party application, the third-party application first obtain a temporary ('request') token from the service provider via a request certified with these keys. 

This request token's only purpose is for the third-party authorization request. The third-party app redirects the user to the service provider's authentication process along with this token, and after authentication, the user is prompted to grant the permissions requested by the third-party app. 

After the user grants permissions, they are redirected back to the client application, along with a verification code and the request token in the redirect URL. The client app sends the verification code with the request token, along with ceritifying the request using the Consumer Secret, and the service provider verifies these and then returns an access token a long with an Access Token Secret. 

Now the access token has been granted to the client, each subsequent request is made with this access token. For each request, the client provides a signature generated from the Consumer Secret and the Access Token Secret.

Confused? Me too. Even in this description of the OAuth 1.0 flow, I've left out several other fields required in some requests. This complexity is one of a number of OAuth 1.0 drawbacks, here are a few more:

1) Complexity - The number of keys, secrets, signatures, and amount of cryptography involved means that incorrect implementations may have vulnerabilities, and even tested third-party libaries may be vulerable if used incorrectly.

2) Client-side secret storage - each request from the client-app requires secrets stored on the user device

3) In the request verification process, secrets need to be shared between the server and the client - this needs to be accounted for when servers are scaled horizontally.

...
During registration the app is assigned a 'client_id', a unique identifer for the app, as well as a registered redirect URI pointing to the client app that ensures the authorization server will send the user back to the genuine client app after authorization.

2) When a user wishes to authorize the client app, they are directed to the authorization server with a URL with the following query parameters:
...
'state' - a randomly value generated by the client app that is sent back by the server in the response. Before navigating to the authentication server with the state value, it should be stored in the user's browser as a HttpOnly cookie. The state value is used to protect against CSRF attacks. 

A CSRF attack, by definition, tricks the user into initiating a request from a location outside of the genuine client site, such as from within an email or another site. Since the state value is generated by the genuine client site for each user session, when the same value is returned by the authentication server it confirms the authorization response is for the same authentication request that was initiated by the user.

While the client_id and redirect_uri also to some degree confirm to the authentication server that the authentication request came from a valid source, they are at risk of being intercepted by an attacker and used to mimic an genuine request. It is only when the client application compares the state value from the authorization response to the one it first generated that the response can be truly verified as genuine and the authorization flow can proceed.

...
The PKCE token exchange mechanism can also return refresh tokens giving the same [user-experience benefits](/_posts/2023-04-01-oauth2-auth-code-flow#maintaining-client-app-authorization) as server-based Authorization Code Flow.
...
5) **Accessing Protected Resources**
    ...
    If supported by the OAuth 2.0 implementation, the same [Refresh Token mechanism](/_posts/2023-04-01-oauth2-auth-code-flow#obtaining-fresh-access-tokens-with-a-refresh-token) outlined for regular Authorization Code Flow can be used to gain new Access Tokens when they near expiry. 
